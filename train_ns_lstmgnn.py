...
class Model(pl.LightningModule):
    def __init__(self, config, dataset, train_loader, subgraph_loader, eval_split='test', \
        ...
        self.per_node_metrics = lambda truth, pred: get_per_node_result(truth, pred, self.dataset.idx_test,
    ...
    def validation_step(self, batch, batch_idx):
        ...
        out, out_lstm = self.net.inference(x, flat, edge_weight, self.ts_loader, self.subgraph_loader,
                                           self.device)  # within this - loop the entire subgraph loader
    ...
    def test_step(self, batch, batch_idx):
        ...
        if self.get_emb or self.get_logits:
            ...
            if self.get_attn:
                ...
                out, out_lstm, edge_index_w_self_loops, all_edge_attn = self.net.inference_w_attn(x, flat, edge_weight,
                                                                                                  edge_index,
                                                                                                  self.ts_loader,
                ...
                results = {'hid': out, 'edge_index': edge_index_w_self_loops, 'edge_attn_1': all_edge_attn[0],
                           'edge_attn_2': all_edge_attn[1]}
            else:
                if self.get_logits:
                    out, out_lstm = self.net.inference(x, flat, edge_weight, self.ts_loader, self.subgraph_loader,
                                                       self.device)  # within this - loop the entire subgraph loader
...
def get_data(config, us=None, vs=None):
    ...
    sample_sizes = [config['ns_size1']] if config['gnn_name'] == 'mpnn' else [config['ns_size1'] + config['ns_size2'],
                                                                              config['ns_size1']]
...
def main_test(hparams, path_results=None):
    ...
    model, config, dataset, train_loader, subgraph_loader = Model.load_model(log_dir, data_dir=hparams['data_dir'],
                                                                             graph_dir=hparams['graph_dir'], \
                                                                             multi_gpu=hparams['multi_gpu'],
                                                                             num_workers=hparams['num_workers'])
    trainer = pl.Trainer(
